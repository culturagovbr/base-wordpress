...
class STC_Subscribe
{
    ...
    public function resend_post_option()
    {
        ...
        <div class="misc-pub-section stc-section">
            ...
            <div id="stc-resend-info" style="display:none;">
                <p>
                    <i><?php printf(__('This post update will be re-sent to subscribers %s', 'stc_textdomain'), $next_run); ?></i>
                </p>
            </div>
        </div>
        <?php
    ...
    public function exclude_from_send_post_option()
    {
        ...
        <div class="misc-pub-section stc-section">
            ...
            <label>
                <input id="stc-resend" type="checkbox"
                       name="stc_block_from_sending" <?php echo ($stc_status == 'blocked') ? 'checked="checked"' : ''; ?>> <?php _e('Remove post from send', 'stc_textdomain'); ?>
            </label>
        </div>
        <?php
    }
    ...
    public function unsubscribe_html()
    {
        ...
        <div id="stc-unsubscribe-wrapper" class="">
            <div class="alert alert-success text-center">
                ...
                <p>
                    <a href="<?php echo get_bloginfo('url'); ?>"><?php _e('Take me to start page', 'stc_textdomain'); ?></a>
                </p>
            </div>
        </div>
        <?php
    ...
    private function email_html_unsubscribe($stc = '')
    {
        ...
        <div style="margin-top: 20px;"><a
                    href="<?php echo get_bloginfo('url') . '/?stc_unsubscribe=' . $stc['hash'] . '&stc_user=' . $stc['email'] ?>"><?php _e('Follow this link to confirm your unsubscription', 'stc_textdomain'); ?></a>
        </div>
        <?php
                ...
                <?php else: ?>
                    ...
                    <form role="form" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" id="stc-email" class="form-control" name="stc_email"
                        ...
                        <div class="form-group">
                            ...
                            <!--<input type="text" id="stc-city" class="form-control" name="stc_city" value=""
                                   placeholder="<?php /*_e('Município', 'stc_textdomain'); */?>"/>-->
                            <select id="stc-city" class="form-control" name="stc_city">
                        ...
                        <div class="checkbox">
                            <label>
                                <input type="checkbox" id="stc-unsubscribe-checkbox" name="stc-unsubscribe"
                                       value="1" <?php checked('1', $post_stc_unsubscribe); ?>>
                            ...
                            <?php if (!empty($cats)) : ?>
                                <?php if (count($cats) > 1) : ?>
                                    ...
                                    <?php if ($this->show_all_categories === true) : ?>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" id="stc-all-categories" name="stc_all_categories"
                                                       value="1">
                                ...
                                <div class="stc-categories-checkboxes">
                                    <?php if (count($cats) > 1) : ?>
                                        <?php foreach ($cats as $cat) : ?>
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" name="stc_categories[]"
                                        ...
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <input type="hidden" name="stc_categories[]"
                                               value="<?php echo $cats[0]->cat_ID ?>">
                                    <?php endif; ?>
                        ...
                        <button <?php echo ($this->settings['acceptance_terms']) ? 'disabled="true"' : ''; ?>
                                id="stc-subscribe-btn" type="submit"
                                class="btn btn-default"<?php echo $post_stc_unsubscribe == 1 ? ' style="display:none;"' : NULL; ?>><?php _e('Subscribe me', 'stc_textdomain') ?></button>
                        <button id="stc-unsubscribe-btn" type="submit"
                                class="btn btn-default"<?php echo $post_stc_unsubscribe != 1 ? ' style="display:none;"' : NULL; ?>><?php _e('Unsubscribe', 'stc_textdomain') ?></button>
                    </form>
                <?php endif; ?>
